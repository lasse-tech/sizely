...
to. Two ways around it, both verified:

* <kbd>Alt</kbd>+<kbd>Space</kbd> opens Muffin's window menu for any window,
  decorated or not, and Sizely's entries are in there.
* The centering shortcut works regardless — it acts on
  `global.display.get_focus_window()`, which does not care about decorations.
